...
/// struct for typed errors of method [`locate_local_pending_object`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LocateLocalPendingObjectError {
    Status422(crate::models::HttpValidationError),
    UnknownValue(serde_json::Value),
}
...
pub async fn locate_local_pending_object(configuration: &configuration::Configuration, locate_object_request: crate::models::LocateObjectRequest) -> Result<i32, Error<LocateLocalPendingObjectError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/locate_physical_object", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<LocateLocalPendingObjectError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
