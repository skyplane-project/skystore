...
class Server:
    ...
    def start_skystore_gateway(self, region: str, gateway_docker_image: str, log_viewer_port=8888):
        ...
        gateway_daemon_cmd = f"/etc/init.d/stunnel4 start && python -u /pkg/skyplane/skystore/gateway/server_daemon.py --region {region}"
    ...
    def start_gateway(
        ...
        try:
            logging.disable(logging.CRITICAL)
            wait_for(is_api_ready, timeout=120, interval=0.1, desc=f"Waiting for gateway {self.uuid()} to start")
        except TimeoutError as e:
            logger.fs.error(f"Gateway {self.instance_name()} is not ready {e}")
            logger.fs.warning(desc_prefix + " gateway launch command: " + docker_launch_cmd)
            logs, err = self.run_command(f"sudo docker logs skyplane_gateway --tail=100")
